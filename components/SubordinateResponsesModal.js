...
export default function SubordinateResponsesModal({
  open,
  onClose,
  questionnaireId,
}) {
  ...
  async function load() {
    try {
      ...
      const res = await fetch(
        `/api/evaluations/rm/self-responses/${questionnaireId}`,
        {
          headers: {
      ...
      console.error("Failed to load subordinate responses:", error);
      setData({
        responses: [],
        feedback: "",
