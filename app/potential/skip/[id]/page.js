...
export default function SkipPotentialPage() {
  ...
  useEffect(() => {
    async function loadForm() {
      try {
        const res = await fetch(`/api/scoring/potential/skip/form/${assessmentId}`, {
          headers: {
  ...
  async function saveDraft() {
    try {
      ...
      const res = await fetch("/api/scoring/potential/skip/save", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async function submitForm() {
    try {
      ...
      const res = await fetch("/api/scoring/potential/skip/submit", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
      ...
      toast.success("Skip potential questionnaire submitted.");
      ...
      <main className="min-h-screen bg-[#F3F4F6]">
        ...
        <section className="mx-auto max-w-5xl px-6 py-8">
          <div className="rounded-[28px] bg-white p-6 shadow-[0_12px_30px_rgba(0,0,0,0.08)]">
            <p className="text-sm text-gray-600">Loading Skip potential form...</p>
          </div>
        </section>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-[#F3F4F6]">
      ...
      <section className="mx-auto max-w-5xl px-6 py-8">
        <div className="mb-8 rounded-[28px] bg-white p-6 shadow-[0_12px_30px_rgba(0,0,0,0.08)]">
          <h1 className="text-3xl font-semibold tracking-tight text-[#111827]">
            Skip-Level Potential Questionnaire
          </h1>
      ...
      <PotentialSubmitConfirmModal
        ...
        evaluatorLabel="Skip-Level Manager"
        loading={submitting}
