...
import SkipSelfResponsesModal from "@/components/SkipSelfResponsesModal";
import SkipRMResponsesModal from "@/components/SkipRMResponsesModal";
...
function formatSkipQuestion(text, name) {
  if (!text) return "";
            ...
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div className="max-w-3xl">
                <p className="text-sm font-medium leading-7 text-[#111827]">
                  Q{index + 1}. {formatSkipQuestion(q.question_text, employeeName)}
                </p>
              </div>
...
export default function SkipReviewPage() {
  ...
  async function loadForm() {
    try {
      const res = await fetch(`/api/evaluations/skip/review-form/${questionnaireId}`, {
        headers: {
      ...
      if (!res.ok) {
        throw new Error(data?.error || data?.detail || "Failed to load skip review form.");
      }
      ...
      console.error(error);
      toast.error(error.message || "Failed to load skip review form.");
      setQuestions([]);
    } finally {
      setLoading(false);
    }
  ...
  async function saveDraft() {
    try {
      ...
      const res = await fetch("/api/evaluations/skip/save-draft", {
        method: "POST",
        headers: {
  ...
  async function submitReview() {
    try {
      ...
      const res = await fetch("/api/evaluations/skip/submit", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
      ...
      if (!res.ok) {
        throw new Error("Failed to submit skip review");
      }

      toast.success("Skip review submitted successfully.");
      ...
      console.error(error);
      toast.error(error.message || "Failed to submit skip review");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#F3F4F6]">
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-[#111827]">
              Skip-Level Review Questionnaire
            </h1>
            <p className="mt-2 text-sm text-gray-600">
              Review the employee questionnaire after RM assessment and provide your final manager rating.
            </p>
            {department ? (
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="rounded-xl bg-[#111827] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:shadow-md"
            >
              View Reporting Manager Responses
            </button>
          </div>
        </div>
      ...
      <SkipSelfResponsesModal
        open={selfModalOpen}
        onClose={() => setSelfModalOpen(false)}
        questionnaireId={questionnaireId}
        />

      <SkipRMResponsesModal
        open={rmModalOpen}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-[28px] bg-white p-6 shadow-2xl">
            ...
            <p className="mb-6 text-sm leading-6 text-gray-600">
              Are you sure you want to submit this skip-level review?
              <br />
